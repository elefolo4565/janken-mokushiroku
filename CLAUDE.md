...
## 勝負ゾーンシステム
- **接触バトル廃止**: フィールド上での接触によるじゃんけんは廃止。代わりに「勝負ゾーン」方式を採用
...
- **BottomCommandBar廃止**: 手の選択はゾーン内UIのみ。交渉ボタンは独立した小ボタンとして残存
...
- **GDScript型推論エラー**: `event.position`はVariant型を返すため、`var diff: Vector2 = Vector2(event.position) - center` のように明示的にキャストが必要
- **ゾーン離脱時のクリーンアップ**: ゾーンマッチ中のプレイヤーが退場/切断した場合、相手にZONE_CANCELLEDを送信しゾーン状態をリセットする必要がある
- **AI zoneFightChoice**: AIはマッチング後1回だけ判断（`_zoneDecisionMade`フラグ）。判断結果はzoneFightChoiceに設定するか、`{ type: 'leave' }`を返してゾーン離脱
